...
FwOnlinePlanarRectification::FwOnlinePlanarRectification(
  ...
  Eigen::VectorXd k = ncameras_->getCameraShared(kFrameIdx)->getDistortion().getParameters();
  std::cout << "k = " << k.transpose() << std::endl;
  undistorter_.reset(new dense::Equidistant_Undistorter(K_, k(0), k(1), k(2), k(3)));
  T_B_C_ = ncameras_->get_T_C_B(kFrameIdx).inverse();
  std::cout << "T_B_C = " << std::endl << T_B_C_.getTransformationMatrix() << std::endl;
  ...
  pub_point_cloud_ = node_handle_.advertise<sensor_msgs::PointCloud2>("/planar_rectification/point_cloud",1);

  // Create a planar rectification parameter object with default values
  dense::PlanarRectificationParams::Ptr parameters (new dense::PlanarRectificationParams);

  // Adapt the parameters according to the desired values
  node_handle_.param<bool>("showImages", parameters->showImages, false);
  node_handle_.param<bool>("allowShiftOfRectImgPlanes", parameters->allowShiftOfRectImgPlanes, true);
  ...
  node_handle_.param<int>("numberOfDisparities", parameters->numberOfDisparities, 80);
  ...
  node_handle_.param<int>("textureThresholdBM", parameters->textureThresholdBM, 20);
  node_handle_.param<int>("uniquenessRatioBM", parameters->uniquenessRatioBM, 80);
  node_handle_.param<int>("speckleWindowSizeBM", parameters->speckleWindowSizeBM, 100);
  node_handle_.param<int>("speckleRangeBM", parameters->speckleRangeBM, 5);
  node_handle_.param<int>("SADWindowSizeSGBM", parameters->SADWindowSizeSGBM, 9);
  node_handle_.param<int>("P1SGBM", parameters->P1SGBM, 120);
  node_handle_.param<int>("P2SGBM", parameters->P2SGBM, 250);
  node_handle_.param<int>("disp12MaxDiffSGBM", parameters->disp12MaxDiffSGBM, 50);
  node_handle_.param<int>("preFilterCapSGBM", parameters->preFilterCapSGBM, 31);
  node_handle_.param<int>("uniquenessRatioSGBM", parameters->uniquenessRatioSGBM, 10);
  node_handle_.param<int>("speckleWindowSizeSGBM", parameters->speckleWindowSizeSGBM, 100);
  ...
  // Create a planar rectification object and initialize it with given parameters.
...
void FwOnlinePlanarRectification::loadCameraRig(std::string ncameras_yaml_path_filename) {
...
void FwOnlinePlanarRectification::addFrame(const aslam::Transformation& T_G_B, const cv::Mat& image_raw) {
  VLOG(0) << "add frame";
     const ros::Time time1 = ros::Time::now();
  if (first_frame_) {
    Eigen::Matrix3d K_new;
    cv::Mat image_undistorted;
    undistorter_->undistortImage(image_raw, downsample_factor_, &K_new, &image_undistorted);
    T_G_B_last_ = T_G_B;
...
//  std::cout << "c = " << c.transpose() << ", c_last = " << c_last.transpose() << std::endl;
...
//            << image_undistorted.type() << std::endl;
  ...
  if (stereoPair->vF1->isDensified()){
    ...
    stereoPair->vF1->getAvgSceneDepth(zAvg);
     ...
     Utils::convertCvPclToRosPCL2Msg(point_cloud, point_cloud_intensity, "/world",
...
//       std::cout << Eigen::Vector3d(cloud[i].x, cloud[i].y, cloud[i].z).transpose() << std::endl;
//     }

     // Compute the angle between the baseline and the optical axis of each vision frame of the stereo pair.
     ...
     float angle_1 = (std::acos(baseline.dot(optical_axis_vF1)) * 360) / (2 * M_PI);
     float angle_2 = (std::acos(baseline.dot(optical_axis_vF2)) * 360) / (2 * M_PI);

     std::stringstream ss;
     ss << ">> Stereo Pair Info:" << std::endl
        << "Image Size: " << stereoPair->vF1->image_.size() << std::endl
        << "Baseline  : " << stereoPair->baseline << " m" << std::endl
        << "Angle between optical axis of vision frame 1 and baseline : " << angle_1 << " °" << std::endl
        << "Angle between optical axis of vision frame 2 and baseline : " << angle_2 << " °" << std::endl
        << "Angle between optical axes: " <<
                   (std::acos(stereoPair->dotOpticalAxes) * 360.0) / (2.0 * M_PI) << " °" << std::endl
        << "Epipole1: " << std::endl << stereoPair->e1.hnormalized() << std::endl
        << "Epipole2: " << std::endl << stereoPair->e2.hnormalized() << std::endl
        << std::endl
...
//  for (std::deque<dense::VisionFrame::Ptr>::iterator i = buffer_->getBeginIterator();
...
//  cv::Mat_<cv::Vec3f> point_cloud;
//  cv::Mat point_cloud_intensity;
//  fused_vision_frame->getPointCloud(stereoPair->vF1->c_, stereoPair->vF1->R_W_C_,
//                                 point_cloud, point_cloud_intensity);
...
void FwOnlinePlanarRectification::showUndistortedCvWindow(const cv::Mat& image_undistorted) {
  cv::imshow("Undistorted image", image_undistorted);
  cv::waitKey(1);
}

void FwOnlinePlanarRectification::publishUndistortedImage(const cv::Mat& image) {
