...
void Utils::convertCvPclToRosPCL2Msg(const cv::Mat_<cv::Vec3f>& cvPcl, const cv::Mat& cvPclIntensity, const std::string& referenceFrameName, const ros::Time& timestamp, sensor_msgs::PointCloud2& rosPcl2Msg){
	...
	rosPcl2Msg.point_step = 16;                                         //Length of a point in bytes float32->32bit=4byte
	rosPcl2Msg.row_step = rosPcl2Msg.point_step * rosPcl2Msg.width;     //Length of a row in bytes
	rosPcl2Msg.data.resize(rosPcl2Msg.row_step * rosPcl2Msg.height);    //Actual point data, size is (row_step*height)
...
//			  std::cout << "x/y/z = " <<  cvPcl.operator()(v, u)[0] << "/"
//						<< cvPcl.operator()(v, u)[1] << "/" << cvPcl.operator()(v, u)[2] << std::endl;
//                std::cout << "cvPcl.operator()(v, u)[0] = " << cvPcl.operator()(v, u)[0] << std::endl;
...
//                const double z = cvPcl.operator()(v, u)[2] - z_origin;
				memcpy(&rosPcl2Msg.data[offset + 0],
						&cvPcl.operator()(v, u)[0], sizeof(float));  //x
				memcpy(&rosPcl2Msg.data[offset + 4],
						&cvPcl.operator()(v, u)[1], sizeof(float));  //y
				memcpy(&rosPcl2Msg.data[offset + 8],
                        &cvPcl.operator()(v, u)[2], sizeof(float));  //z
				...
				memcpy(&rosPcl2Msg.data[offset + 12], &badPoint,
						sizeof(float));
...
void Utils::convertDispMapToRosMsg(const cv::Mat& disparityMap, const std::string& referenceFrameName, const ros::Time& timestamp, stereo_msgs::DisparityImage& disparityRosMsg){
	...
	disparityRosMsg.image.data.resize(
    disparityRosMsg.image.height * disparityRosMsg.image.step);


	//Find Maximum Disparity
	float maxDisp=*std::max_element(disparityMap.begin<float>(),disparityMap.end<float>());
	float minDisp=*std::min_element(disparityMap.begin<float>(),disparityMap.end<float>());
	...
	cv::Mat_<float> disparityImageMsg(
			disparityRosMsg.image.height,
			disparityRosMsg.image.width,
