...
//  std::cout << result_mask_.size() << ", " << image_warped_mutable.size() << std::endl;

//  std::cout << result_mask_.type() << ", " << result_mask_.type() << std::endl;
//  cv::Mat mask_image;
...
void FwOnlineOrthomosaic::updateOrthomosaic(const aslam::Transformation& T_G_B,
  ...
  for (int border_pixel_index = 0; border_pixel_index < border_keypoints_.cols();
    ...
    const double scale = - (T_G_C.getPosition()(2)-414) / (T_G_C.getRotationMatrix() * C_ray)(2);
...
void FwOnlineOrthomosaic::batch(std::vector<kindr::minimal::QuatTransformation> T_G_Bs,
