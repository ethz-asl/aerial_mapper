...
void OrthoBackwardGrid::processIncremental(const Poses& T_G_Cs,
  ...
  for (size_t i = 0u; i < images.size(); ++i) {
    ...
    for (int border_pixel_index = 0;
      ...
      const double scale =
          -(T_G_C.getPosition()(2) - settings_.orthomosaic_elevation_m) /
      ...
      if (map.at("observed", *it) == 0) {
        ...
        if (keypoint_visible) {
          ...
          const int kp_x = std::min(static_cast<int>(std::round(keypoint(0))),
                                    static_cast<int>(camera.imageWidth()) - 1);
          const double gray_value = images[i].at<uchar>(kp_y, kp_x);
          map.at("ortho_r", *it) = map.at("ortho_r", *it) =
...
void OrthoBackwardGrid::printParams() const {
  ...
  std::cout << std::string(50, '*') << std::endl
            ...
            << std::setw(nameWidth) << " - Mode: " << std::left
            << std::setw(nameWidth) << mode << std::endl << std::left
